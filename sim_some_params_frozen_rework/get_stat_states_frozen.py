# The intention is to make a program capable of simulating [QUECHED] for any number of sites
...
if os.path.exists(extSSDpath):
    path = extSSDpath + getProjectFoldername() + '/sim_some_params_frozen_rework/results'
else:
    path = '/results'
...
def simEvoQuenched_iter_lambda(pis, qs, ls, dl, Nsites, N, ar, ir, er, push, ic, max_time, Nrea, ow_geq_Nrea=True, ow_geq_simTime=False, ow_hard=False):
    ...
    '''
    resFile = f'resultsQuenched_sim_Nsites_{Nsites}.csv'
    wd = os.getcwd()
    ...
    change_sim_input(froute, fin_file, pis=pis, qs=qs, max_time=max_time, N_sites=Nsites, N_bots=N, 
                     bots_per_site=bots_per_site, arena_r=ar, interac_r=ir, exclusion_r=er, push=pushFortran)
    all_ls = np.arange(ls[0], ls[1]+dl, dl)
    all_ls = np.around(all_ls, len(str(dl).split('.')[-1]))
    # prepare the results dictionary (later df) PART1
    colOrder = ['N','arena_r', 'interac_r', 'exclusion_r', 'push']
    ...
    results['arena_r'] = [ar]*N_stored_results
    results['interac_r'] = [ir]*N_stored_results
    results['exclusion_r'] = [er]*N_stored_results
    results['push'] = [push]*N_stored_results
    ...
    if os.path.exists(path + '/' + resFile):
        for index,row in df_new.iterrows():   
            bool_series = (df_old['N']==row['N']) & (df_old['arena_r']==row['arena_r']) & (df_old['interac_r']==row['interac_r']) & (df_old['exclusion_r']==row['exclusion_r']) & (df_old['l']==row['l']) & (df_old['ic'] == row['ic'])
            for i in range(1,Nsites+1):
        ...
        df_old = df_old.sort_values(by=['N', 'arena_r', 'interac_r', 'exclusion_r', 'push', f'q{Nsites}', f'pi{Nsites}','l', f'q{Nsites-1}'], ignore_index=True)
...
if __name__ == '__main__':
    ...
    pis, qs, ls, dl, N, arena_r, interac_r, exclusion_r, push, ic, max_time, Nrea = args.pis, args.qs, args.ls, args.dl, args.N, args.arena_r, args.interac_r, args.exclusion_r, args.push, args.ic, args.max_time, args.Nrea
    if len(pis) != len(qs):
    ...
    simEvoQuenched_iter_lambda(pis, qs, ls, dl, Nsites, N, arena_r, interac_r, exclusion_r, push, ic, max_time, Nrea, ow_hard=True) # , ow_hard=True