...
parser.add_argument('q1', type=float, help='site 2 quality')
parser.add_argument(
...
pi1, pi2, q1, x, mu = args.pi1, args.pi2, args.q1, args.x, args.mu
...
# iteratively go through all q2s and find the lambda, if it exits; else NaN
q2_lims = (1.0, 2.0*q1)
dq2 = 0.2
Nq2s = int((q2_lims[1] - q2_lims[0])/dq2) + 1
q2s = np.linspace(q2_lims[0], q2_lims[1], Nq2s)
q2s = np.around(q2s, 2)
...
df = pd.DataFrame({'q2':q2s, 'lambda':lambdas})
...
df.to_csv(f'{path}/Tline_asym_fixq1_q1_{q1}_pi1_{pi1}_pi2_{pi2}_f2_{int(x)}f1.csv', index=False)