...
if ic=='N':
    ...
    bots_per_site = [int(round(0.14*N)), int(round(0.43*N)), int(round(0.43*N))] # probably int() is not necessary
...
filename = newFolderName + '_Euler.csv'
df.to_csv(filename, index=False)
call(f'mv {filename} {path}', shell=True)