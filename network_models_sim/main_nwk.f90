...
program main
    ...
    character(5) :: file_id, config_file_id, Nstr, gamma_str, minDeg_str
    ...
    if (nw_model.eq.'UCM') then
        write(gamma_str, '(F5.1)') nw_param
        write(minDeg_str, '(I5)') int(nw_param_2)
        call init_path()
        call execute_command_line('ls '//trim(adjustl(path))//'N'//trim(adjustl(Nstr))//'_g'//trim(adjustl(gamma_str))//&
        '_min'//trim(adjustl(minDeg_str))//'/ | wc -l > foo&')
    ...
    do i=1,N_rea
        ...
        call generate_network(config_int)
