...
for i,ir in enumerate(interac_r):
    ...
    ax[0].plot(dfir['lambda'], dfir['f0'], label=f'{ir}', color=colors[i])
    ax[1].plot(dfir['lambda'], dfir['f1'], color=colors[i])
    ax[2].plot(dfir['lambda'], dfir['f2'], color=colors[i])
...
fig.tight_layout()
fig.savefig(f'compare_ir_pi1_{pis[0]}_pi2_{pis[1]}_q1_{qs[0]}_q2_{qs[1]}_ar_{arena_r}_many_ir_er_{exclusion_r}_NOPUSH.png')
plt.close(fig)
